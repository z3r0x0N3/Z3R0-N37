...
class Client:
    ...
    def request_lock_cycle_payload(self) -> dict:
        ...
        if not response:
            print("[!] Empty response from primary node when requesting lock-cycle payload.")
            return {}
    ...
    def send_data_through_distributed_proxy_chain(self, original_data: bytes, proxy_chain_config: Dict[str, Any], final_destination: str = None) -> bytes:
        ...
        proxy_chain_config = proxy_chain_config or {}
        node_order = proxy_chain_config.get("node_order") or []
        node_configs = proxy_chain_config.get("node_configs") or {}

        if not node_order or not node_configs:
            print("[DEBUG] Client: Proxy chain config missing; using direct connection fallback.")
            return original_data

        if not node_order:
            self.logger.warning("Proxy chain not yet provisioned; sending data directly to primary node.")
            return original_data
        ...
        for i in reversed(range(len(node_order))):
            ...
            node_info = node_configs.get(node_id, {})
            node_pubkey_pem = node_info.get("pgp_pubkey")
            node_onion_address = node_info.get("onion_address") or node_info.get("onion")

            if not node_pubkey_pem or not node_onion_address:
                print(f"[DEBUG] Client: Node {node_id} missing routing info; aborting chain and returning original data.")
                return original_data
            ...
            if i < len(node_order) - 1: # If not the last node
                ...
                next_node_info = node_configs.get(next_node_id, {})
                next_hop_onion = next_node_info.get("onion_address") or next_node_info.get("onion")
                next_hop_pubkey_pem = next_node_info.get("pgp_pubkey")
        ...
        first_node_info = node_configs.get(first_node_id, {})
        first_node_onion = first_node_info.get("onion_address") or first_node_info.get("onion")
        if not first_node_onion:
            print("[DEBUG] Client: First node missing onion address; returning original data.")
            return original_data
